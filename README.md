...
SRC="/mnt/mmc01/logo_2820.yuv"
DST="/tmp/fuse_d/DCIM_RES/logo_2820.yuv"
BACKUP="/tmp/fuse_d/DCIM_RES/logo_2820.yuv.bak"
LOG="/tmp/auto_logo.log"

echo "[auto_logo] start $(date)" >> "$LOG"

if [ ! -f "$SRC" ]; then
  echo "[auto_logo] source logo not found: $SRC" >> "$LOG"
  exit 0
fi

if [ ! -f "$BACKUP" ] && [ -f "$DST" ]; then
  cp "$DST" "$BACKUP"
  echo "[auto_logo] backup created" >> "$LOG"
fi

cp "$SRC" "$DST"
sync
echo "[auto_logo] logo updated" >> "$LOG"
```
